...
int Open(const char* filename, FILE* file_descriptor, int flags)
{
	...
	if ((flags == O_RDONLY) || (flags == O_RDWR)) //read only mode
	{
		if (file1)
			file1->offset = 0;
		else
			return -1; 		//file doesnt exist, read only mode.
	}
	else if ((flags == (O_WRONLY | O_CREATE)) || flags == (O_APPEND | O_CREATE)) //write only mode.
	{
		if (!file1)
		{
			createFile(filename);
		...
		if (flags & O_WRONLY)
			file1->fileSize = 0;
		file1->offset = file1->fileSize;
	}
	else if ((flags == (O_RDWR | O_CREATE)) || (flags == (O_APPEND | O_CREATE | O_RDONLY)))
	{
		if (!file1)
		{
			createFile(filename);
		...
		if (flags == (O_RDWR | O_CREATE)) //first case
			file1->fileSize = 0;
		file1->offset = file1->fileSize;
	}
...
{
	MyFile* file1 = (MyFile*)file;
	if (file1->flags & (O_WRONLY | O_RDWR | O_APPEND))
